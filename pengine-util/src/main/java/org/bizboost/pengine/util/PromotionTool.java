...
public class PromotionTool {
    ...
    public final static Rule convertToRule(String conditionStr) throws IllegalRuleFormat {
        ...
        if(firstSpaceIndex==-1||firstSpaceIndex==0) throw new IllegalRuleFormat(format("非法促销条件格式:%s, 格式要求:'<真|假> <判断条件>'",conditionStr));
        String exactMatchStr = StringUtils.trimWhitespace(conditionStr.substring(0,firstSpaceIndex));
        if (!exactMatchStr.equals(Boolean.TRUE.toString())&&!exactMatchStr.equals(Boolean.FALSE.toString())){
            throw new IllegalRuleFormat(format("非法促销条件格式:%s, 格式要求:'<真|假> <判断条件>'",conditionStr));
        }
